...
- **If the source is already inside a component's `enablement/` directory** (e.g., a `final.md` from a prior `blog-create` run), write the preview as `index.html` alongside it — no further questions needed.
- **Otherwise** (pasted text, a URL, a Google Doc, or a markdown file that isn't yet filed anywhere), determine which component partition in `components/components.yaml` this content belongs to — infer from the topic and confirm, or ask if it's unclear. Output goes to `components/<component>/enablement/blog-<topic-short>/index.html`, creating the directory if it doesn't exist yet. When creating the directory, also scaffold an `artifact.md` descriptor (`type: artifact`, `title`, one-line `description`, `timestamp`, `components:` spread) — `views/artifacts.md` flags descriptor-less directories. If no existing component fits, don't invent one — hand off to `hub.file`, then resume once it exists.
