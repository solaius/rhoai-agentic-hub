...
3. Write components/<c>/knowledge/ref-<slug>.md — or
...
4. Transcripts: raw file → components/<c>/work/transcripts/ (or
...
5. NDA-adjacent → the restricted/ mirror of the chosen home —
   restricted/components/<c>/knowledge/ or restricted/narrative/knowledge/
   (same shapes).
...
8. Commit: stage the filed entry (+ components.yaml if a partition was
   ...
   `git commit -m "know(<c>): file <slug>" -- <those paths>` && `git push`.
