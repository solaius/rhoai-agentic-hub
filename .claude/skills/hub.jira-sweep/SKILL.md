---
name: hub.jira-sweep
description: Sweep Jira into the hub for one component — conversational scope discovery (JQL stored in components.yaml), a curated public snapshot under <component>/work/, and gated ref- entries for strategic issues (default Outcome/Feature types). Use when the user says "sweep jira for <component>", "pull the jiras for <component>", "set up jira tracking for <component>", or when filed Jira links deserve field ingestion. Read-only against Jira; every repo write is gated.
---

# hub.jira-sweep

Input: a component id (components/components.yaml) + optional JQL/scope hints.
...
2. SCOPE: a `jira:` block on the component in components/components.yaml → use
...
4. DRAFT (scratch only, no repo writes): one ref- entry per candidate the
   component does not already track (match on resource: URL) — filename
   ...
   why it matters to this component). An existing ref- for a candidate →
   ...
   marked public: false in candidates-<component>.yaml are exactly those —
...
6. On OK: write the approved files, `python scripts/hub_index.py`, then
   ...
   `git commit -m "jira(<component>): sweep - <n> issues, <m> refs" -- <those paths>`
   && `git push`.
