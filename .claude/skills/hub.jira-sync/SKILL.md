...
2. DIFF: `python scripts/hub_jira.py --sync [<component>] --out
   <scratchpad>/jira`. It reads committed snapshots, re-runs stored JQLs,
   and reports NEW / CHANGED / VANISHED per component, plus WATCHED lines
...
3. INTERPRET the diff into proposals (scratch only):
   - the refreshed snapshot per changed component (the CLI already wrote
   ...
   - NEW arrivals whose type is in the component's `ref_types` → ref-
...
5. On OK: write, `python scripts/hub_index.py`, `python
   ...
   `git commit -m "jira(sync): <components> - <summary>" -- <those paths>`
   && `git push`.
