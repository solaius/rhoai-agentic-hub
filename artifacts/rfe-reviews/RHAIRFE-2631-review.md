...
|-----------|-------|-------|
| WHAT      | 2/2   | Clear, specific need: expose OpenShift AI's memory capability through MCP tool calls so any MCP-capable client gets memory without bespoke SDK integration, and so memory is discoverable/governable like the rest of the platform's MCP portfolio. Second concrete need is called out too — correct per-user/agent/project scoping under MCP's stateless connection model. Unambiguous. |
| WHY       | 2/2   | Named account: Garanti BBVA, evaluating MCP tools for governed access to internal data, with memory-as-MCP fitting that same evaluation. Plus a strategic causal chain — an MCP surface means one integration serves every MCP-capable client instead of per-framework work, "directly reducing the cost of the framework-agnostic promise in the parent Outcome," and aligns memory with the existing MCP Catalog/Gateway/AI Asset Registry strategy. Named account plus a clear causal chain tying this capability to a strategic commitment. |
| Open to HOW | 2/2 | MCP is the customer's chosen transport ("customers standardizing on MCP as their agent-tool transport"), so exposing memory via MCP is the need itself, not a prescribed architecture. The AC describe customer-facing surfaces and outcomes — store/search/retrieve via MCP tool calls (API surface = WHAT), catalog discoverability, correct scoping under stateless connections (observable requirement), governance via existing MCP capabilities (business capabilities), and community naming conventions for interop. No internal schemas, repos, or component mandates; engineering retains freedom on implementation. |
| Not a task | 2/2  | A genuine business need — customers standardizing on MCP governance cannot reach platform memory from MCP clients, forcing bespoke per-harness integration and leaving memory outside the governance model. Not a chore, tech-debt, or engineering activity. |
| Right-sized | 2/2 | Deliverables identified: (a) memory reachable via MCP tool calls, (b) catalog discoverability, (c) correct stateless scoping, (d) MCP-style governance (authz/metrics/audit), (e) community-convention tool naming. Independence test: none ships standalone with value — discovery/governance/naming have nothing to act on without the tool surface, and the tool surface is unusable multi-tenant without scoping. All are tightly coupled facets of one capability: memory as a governed MCP tool. Focused single feature. |
| **Total** | **10/10** | **PASS** |

### Verdict
A well-formed, focused RFE that cleanly states a customer-driven need, backs it with a named account and a strategic causal chain, and describes the required MCP surface without prescribing internal architecture.

### Feedback
Strong across all criteria. Minor, optional improvements: the "multiple named accounts on file with the PM" cohort could be made even more concrete by listing one or two of those accounts inline (as it does for Garanti BBVA), which would further harden the WHY. The stateless-scoping acceptance criterion is the most implementation-adjacent item — it is currently framed as an outcome ("scoped correctly ... even when the underlying MCP connection is stateless"), which is correct; keep it outcome-focused and avoid drifting into prescribing a specific identity/session mechanism when this reaches strategy/feature breakdown.
...
The following items are flagged for /strat.refine (none block submission):

1. Backend dependency / sequencing — this RFE is the MCP surface over a memory capability, not the store itself; the store must be delivered by sibling work under parent Outcome RHAISTRAT-1345 before the surface can go end-to-end.
2. "Existing MCP governance capabilities" overstates current maturity — several young, non-uniform mechanisms exist rather than one uniform plane; engineering must choose which one memory-MCP plugs into.
3. Host decision — extend rhoai-mcp (v0.1, Developer Preview, read-only by default) with read-write memory tools vs. stand up a dedicated memory MCP server.
4. Three-way scoping (user + agent + project) crosses two identity planes — OIDC/K8s user identity in rhoai-mcp vs. SPIFFE agent identity in OpenShell (overlay 0015).
5. Catalog registration (AC #2) is a build-time, cross-team content task owned by the model-registry/catalog team.
6. AC #5's community memory-tool conventions are an unstandardized, moving target — an interop maintenance risk to name.
7. External-operator policy (overlay 0008) — any external datastore/operator the backend needs must be a documented DSC/DSCI prerequisite, not auto-installed.
