...
- **No `kubeflow/hub` component doc.** The snapshot's `kubeflow.md` is the *notebook controller*, not the catalog backend. The Agent/MCP/Model catalog backend ([kubeflow/hub](https://github.com/kubeflow/hub), the renamed model-registry — [/components/platform/knowledge/decision-kubeflow-hub-rename.md](/components/platform/knowledge/decision-kubeflow-hub-rename.md)) has no dedicated summary.
- **No AI Hub functional doc** and no agent-catalog BFF — expected: the snapshot (June 22) predates the BFF build, which started after the OpenAPI spec merged 2026-07-03 ([/components/agent-catalog/knowledge/fact-agent-catalog-35-scope.md](/components/agent-catalog/knowledge/fact-agent-catalog-35-scope.md)).
- **It documents Kagenti** (`agents-operator.md`), while the roadmap has since moved the agent-runtime bet to **OpenShell** ([/components/agent-registry/knowledge/fact-kagenti-roadmap-removal.md](/components/agent-registry/knowledge/fact-kagenti-roadmap-removal.md)). Treat the Kagenti doc as design-concept reference, not the 3.6 plan of record.
...
- **AI Hub** is a UI construct on this dashboard, not a component: Catalog / Registry / Deployments tabs per asset type, PatternFly ([/components/platform/knowledge/fact-ai-hub-ui.md](/components/platform/knowledge/fact-ai-hub-ui.md)). Admin-facing governance surfaces live here; consumption lives in Gen AI Studio.
...
- **MaaS.** maas-controller / maas-api / payload-processing on **Envoy Gateway + Kuadrant** (AuthPolicy, TokenRateLimitPolicy, Authorino callbacks). The `ext_proc` pipeline does pre-auth model-name extraction and **post-auth API-format translation + provider credential injection** — an OpenAI-compatible surface (`/v1/models`, `sk-oai-*` keys) that can proxy to external providers. This is the mechanical substrate for the planned **agentic API surface** (Anthropic Messages API + OpenAI Responses API with runtime model-name translation — [/components/agent-catalog/knowledge/fact-agent-deployment-modes.md](/components/agent-catalog/knowledge/fact-agent-deployment-modes.md), [/components/platform/knowledge/ref-open-responses-specification.md](/components/platform/knowledge/ref-open-responses-specification.md)).
...
The Agent Catalog is "v3" of the Model → MCP → Agent catalog pattern (Andrew Ballantyne): kubeflow/hub backend, YAML catalog source baked at build time, AI Hub federated UI ([/components/agent-catalog/knowledge/fact-agent-catalog-overview.md](/components/agent-catalog/knowledge/fact-agent-catalog-overview.md)). In 3.5 it is read-only (cards link out to GitHub, no deploy — [/components/agent-catalog/knowledge/decision-agent-catalog-no-deploy-35.md](/components/agent-catalog/knowledge/decision-agent-catalog-no-deploy-35.md)); at 3.6 EA1 it grows a deploy path.
...
| **mcp-gateway** | Not required; Gen AI Studio reaches MCP servers via the `gen-ai-aa-mcp-servers` ConfigMap ([/components/mcp-gateway/knowledge/fact-mcp-gateway.md](/components/mcp-gateway/knowledge/fact-mcp-gateway.md)) | Deployed agents get gateway-mediated MCP access: Envoy WASM plug-in in the RHCL stack (MCP Router ext_proc → Broker), identity-aware per-tool authz pairing with agent SPIFFE identity; Jehlum's bootstrap proposal preconfigures supported harnesses with a platform skill + the OpenShift AI MCP server "ideally through the MCP Gateway" ([/components/agent-catalog/knowledge/ref-platform-skills-for-harnesses-gdoc.md](/components/agent-catalog/knowledge/ref-platform-skills-for-harnesses-gdoc.md)); Gateway GA rides RHCL 1.5 (Oct 2026) |
| **mcp-lifecycle-operator** | The deploy-side analog: MCPServer CRD (`mcp.x-k8s.io/v1alpha1`) is what "deploy from catalog" already means for MCP servers; TP mid-Aug 2026 | Indirect: catalog-deployed agents consume MCPLO-deployed servers (via gateway); MCPLO GA scope (OCPSTRAT-2879) explicitly adds Agent Sandbox/Code Mode support — the two runtimes meet at the CR layer ([/components/mcp-lifecycle-operator/knowledge/index.md](/components/mcp-lifecycle-operator/knowledge/index.md)). Note the contrast: agents get an SDK-driven deploy (OpenShell), not a CRD-first operator like MCPLO |
| **agent-registry** | None shipped. Semantics agreed in principle: catalog = seeds/templates, a configured deployed instance = a **version** in the registry (Adel — [/components/agent-catalog/knowledge/fact-agent-deployment-modes.md](/components/agent-catalog/knowledge/fact-agent-deployment-modes.md)) | Backend 3.6 EA2 (RHAISTRAT-1436, UI later RHAIRFE-1313), likely MLflow-based (`AgentDiscoveryProvider` poll/watch/webhook could watch AgentSandbox CRs — [/components/agent-registry/knowledge/fact-agent-registry.md](/components/agent-registry/knowledge/fact-agent-registry.md)); whether Deploy becomes "Register" is unresolved (ODH ADR #142 — [/components/agent-catalog/knowledge/question-agent-catalog-register-vs-deploy.md](/components/agent-catalog/knowledge/question-agent-catalog-register-vs-deploy.md)) |
| **agent-interop / OpenShell** | Read-only deployments view discovers AgentSandbox CRs labeled `openshell.ai/managed-by` (agent-ops BFF); OpenShell DP ships in 3.5 ([/components/agent-interop/knowledge/fact-agent-interop-overview.md](/components/agent-interop/knowledge/fact-agent-interop-overview.md)) | The deploy target: BFF → **OpenShell Go SDK** (Gage: the SDK is "our blocker"; [NVIDIA/OpenShell#2044](https://github.com/NVIDIA/OpenShell/issues/2044), rhuss/openshell-sdk-go prototype). OpenShell provides gateway control plane, sandbox, policy engine, privacy router ([NVIDIA/OpenShell](https://github.com/NVIDIA/OpenShell)); identity is SPIFFE/SPIRE with token exchange ([/components/agent-interop/research/00-executive-summary.md](/components/agent-interop/research/00-executive-summary.md)) |
| **gen-ai-studio** | Separate persona surface (AI Engineers), no direct catalog integration; harness-in-playground explicitly not 3.5 (needs session management, HITL, tool approval — [/components/agent-catalog/knowledge/question-agent-catalog-harness-playground-integration.md](/components/agent-catalog/knowledge/question-agent-catalog-harness-playground-integration.md)) | Shares two rails with deployed agents: the MCP Gateway consumption path and the unsolved user-identity flow-through question; its AI Available Assets view is the natural future home for agent endpoints |
| **skills-registry** | Upstream-only (MLflow MVP: SKILL.md + frontmatter, `install_skill()` into harness dirs — [/components/skills-registry/knowledge/index.md](/components/skills-registry/knowledge/index.md)) | Binding target for Mode 2: declared skills resolve against the skills registry; skills ship at most as "tested with these models"; PVC-mounted skills-as-markdown floated as the disconnected workaround ([/components/agent-catalog/knowledge/fact-agent-catalog-36-supported-images.md](/components/agent-catalog/knowledge/fact-agent-catalog-36-supported-images.md)) |
| **model catalog / MaaS** | Pattern v1 (same hub backend); no runtime link | Two links: binding resolves the agent's declared model against the model catalog/MaaS; at runtime the harness talks to the **agentic API surface** (Anthropic Messages + OpenAI Responses) whose translation/credential-injection machinery MaaS `ext_proc` already implements in 3.5-ea.2; catalog cards carry an optional "tested models" field ([/components/agent-catalog/knowledge/decision-agent-catalog-35-field-set.md](/components/agent-catalog/knowledge/decision-agent-catalog-35-field-set.md)) |
...
The container contract predates the flow and survives it: port 8000, `/.well-known/agent-card.json`, non-root (USER 1000), all config via env vars ([/components/agent-catalog/knowledge/fact-agent-catalog-starter-kits.md](/components/agent-catalog/knowledge/fact-agent-catalog-starter-kits.md)). Deploy is restricted to **supported** (platform-built, not vendor-"validated") harness images ([/components/agent-catalog/knowledge/decision-agent-catalog-deploy-supported-images-only.md](/components/agent-catalog/knowledge/decision-agent-catalog-deploy-supported-images-only.md), [/components/agent-catalog/knowledge/decision-supported-not-validated-images.md](/components/agent-catalog/knowledge/decision-supported-not-validated-images.md)), built on AIPCC agent-runtime base images (RHAIRFE-2443; ADR fit still questioned).
...
- /components/agent-catalog/knowledge/ — overview, 35-scope, 36-supported-images, deployment-modes, starter-kits facts; the four decisions; register-vs-deploy, aipcc-fit, harness-playground questions; platform-skills ref
- /components/agent-interop/knowledge/fact-agent-interop-overview.md · ref-declarative-harness-proposal-gdoc.md · /components/agent-interop/research/00-executive-summary.md
- /components/agent-registry/knowledge/index.md (fact-agent-registry, fact-kagenti-roadmap-removal, fact-agentic-base-images)
- /components/mcp-catalog/knowledge/fact-mcp-catalog-overview.md · /components/mcp-registry/knowledge/index.md · /components/mcp-gateway/knowledge/fact-mcp-gateway.md · /components/mcp-lifecycle-operator/knowledge/index.md
- /components/gen-ai-studio/knowledge/index.md · /components/skills-registry/knowledge/index.md
- /components/platform/knowledge/fact-ai-hub-ui.md · decision-registry-vs-catalog.md · ref-open-responses-specification.md
