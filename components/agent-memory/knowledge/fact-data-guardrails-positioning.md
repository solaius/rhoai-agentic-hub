...
This is proposal-stage positioning, not a decision — but it's a plausible lens for the broader registry/governance story beyond agent memory specifically, since the registry-vs-catalog split (see [decision-registry-vs-catalog.md](/components/platform/knowledge/decision-registry-vs-catalog.md)) is itself about where governance sits relative to data/asset flow. See [agent-memory-landscape-research.md](/components/agent-memory/research/agent-memory-landscape-research.md) for the fuller research this framing comes from, and [decision-mcp-gateway-tool-governance-scope.md](/components/mcp-gateway/knowledge/decision-mcp-gateway-tool-governance-scope.md) for a related output-side scope boundary already decided for MCP Gateway.
