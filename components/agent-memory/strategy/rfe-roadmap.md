...
- **Scope:** A context-engineering capability over the substrate — inspectable compaction (human-readable summaries), retrieval assembly, progressive disclosure, KV-cache-aware ordering — now backed by Phase 2 research ([research 13](/components/agent-memory/research/13-kv-cache-optimization.md)). Built on existing compaction primitives (`/responses/compact` and `context_management`). Carries its own quality metric (token efficiency at a quality threshold).
...
- **Problem:** Phase 1 (3.7) designs for the gateway architecture but may use a transitional fallback where gateway primitive parity is incomplete ([research 16](/components/agent-memory/research/16-ai-gateway-memory-substrate.md) §3); FIPS is delegated but not validated end-to-end.
...
- **Problem:** No existing memory system ships with built-in quality measurement ([research 12](/components/agent-memory/research/12-benchmarking-evaluation.md)). Operators cannot validate that the substrate meets quality thresholds before production deployment. Without benchmarks, memory degradation is invisible until agents produce poor results.
...
| Source | Used for |
|---|---|
| [REVIEW-NOTES.md](/components/agent-memory/research/REVIEW-NOTES.md) | D2/D3/D5/D6 — scope, incremental path, standards |
...
| [research 08](/components/agent-memory/research/08-rhoai-ocp-alignment.md) | Productization gaps (D-5/D-6/D-7/D-8/D-9), the Q-G* dependencies |
| [research 09](/components/agent-memory/research/09-agent-harness-memory.md) | Harness memory patterns (RFE-M9 design input) |
| [research 11](/components/agent-memory/research/11-adversarial-memory.md) | Adversarial memory attack surface (RFE-M10) |
| [research 12](/components/agent-memory/research/12-benchmarking-evaluation.md) | Memory quality benchmarking gap (RFE-M11) |
| [research 13](/components/agent-memory/research/13-kv-cache-optimization.md) | KV-cache research backing (RFE-M4) |
| [research 16](/components/agent-memory/research/16-ai-gateway-memory-substrate.md) | Gateway-native transition, primitive mapping, parity gaps (M2, M8) |
