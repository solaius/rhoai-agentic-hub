...
the roadmap removal
([fact-kagenti-roadmap-removal](/components/agent-registry/knowledge/fact-kagenti-roadmap-removal.md)),
...
2. **Agent-card fetch + signature verification has NO successor.** The
   ...
   currently have no producer. Same-day correction (jira-gap lens): the
   former vehicle RHAISTRAT-1956 (mutually-authenticated metadata sync,
   paraphrased) **closed between 2026-07-11 and 2026-07-16** with no
   children and no successor — the whole card chain (1956, 1213, 1599)
   is closed while clone RHAIRFE-2388 remains Approved. Zero active
   Jira paths to `verified=true`; close reason needs an owner check.
   See [research/11-jira-gap](/components/agent-registry/research/11-jira-gap.md).
...
[research/09-architecture](/components/agent-registry/research/09-architecture.md).
