...
Open licensing concern: proprietary harnesses (Claude Code, Antigravity) need one of three paths — a redistribution agreement with Anthropic/Google, an install-at-runtime fallback, or internal-only images (not redistributed); legal involvement is needed to pick a path. Complementary to (does not require) the OpenShell Agent Runtime Contract. Proposal by Adel Zaalouk; AIPCC ownership/scope decision still pending (see [question-agentic-base-images-product-id.md](/components/agent-registry/knowledge/question-agentic-base-images-product-id.md)). UBI-version choice is a separate open question — see [question-agentic-base-images-ubi-version.md](/components/agent-registry/knowledge/question-agentic-base-images-ubi-version.md). See [ref-agentic-base-images-rhairfe-2443.md](/components/agent-registry/knowledge/ref-agentic-base-images-rhairfe-2443.md).
