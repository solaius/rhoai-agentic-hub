...
---
**Llama Stack bridge risk**: [decision-ai-gateway-f2f-architecture.md](/restricted/components/platform/knowledge/decision-ai-gateway-f2f-architecture.md) GAs Llama Stack as a bridge in 3.5 while planning a gateway-native multi-tenant Responses implementation within ~6 months — this dual-track maintenance creates risk that customers build dependencies on the bridge if the replacement slips. The F2F decided the timeline; an execution plan for actually hitting it is still needed.
