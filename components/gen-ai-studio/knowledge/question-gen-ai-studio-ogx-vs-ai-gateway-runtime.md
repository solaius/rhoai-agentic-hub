...
---
Gen AI Studio is currently built on OGX (Red Hat's Llama Stack Distribution) as its AI runtime. [RHAISTRAT-1935](https://redhat.atlassian.net/browse/RHAISTRAT-1935) proposes supporting the AI Gateway (see [fact-ai-gateway.md](/components/ai-gateway/knowledge/fact-ai-gateway.md)) as an alternative backend, targeted for 3.7. Open question: does Gen AI Studio eventually need a pluggable runtime abstraction so it can run against either backend transparently, or will OGX and AI Gateway simply coexist as two named, explicitly-chosen options? This connects to the E2E usability finding that users want to "try a model before infra investment" — a lightweight experimentation path may favor whichever backend is faster to stand up. See the fuller architecture context in [gen-ai-studio-architecture.md](/components/gen-ai-studio/research/gen-ai-studio-architecture.md).
