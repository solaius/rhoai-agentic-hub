...
---
Dependency runs one direction: RHCL does not depend on MCP Gateway, but MCP Gateway depends on RHCL (Kuadrant Operator, Authorino, Limitador — see [fact-mcp-gateway.md](/components/mcp-gateway/knowledge/fact-mcp-gateway.md)).
...
Adjacent packaging concern for a different component: the MCP Lifecycle Operator's own OLM/OperatorHub bundling choice is covered in [ref-rhoai-limited-mcp-lifecycle-faq.md](/restricted/components/mcp-registry/knowledge/ref-rhoai-limited-mcp-lifecycle-faq.md) (restricted).
