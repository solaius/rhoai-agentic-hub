...
```mermaid
flowchart TD
    W["session work"] -->|"hub.capture / hub.file (gated)"| F["components/ - domain knowledge"]
...
generated set: `components/index.md`, every `components/*/index.md` and
`components/*/knowledge/index.md`, `memory/index.md`, and everything in
