...
**Architecture:** All-new files except three surgical touches (`schema.py`, `indexer.py`, `doctor.sh`) and doc edits. The client (`hublib/jira.py`, httpx, async) is read from Jira only; repo writes happen in the skills, gated. Tracked per-feature snapshots (`components/<id>/work/jira-snapshot.yaml`) carry whitelisted fields only, with summaries admitted by the unauthenticated-probe rule; `views/jira-map.md` regenerates deterministically from tracked files, so CI needs no credentials.
...
into components/<id>/work/jira-snapshot.yaml — tracked, public, never
...
SNAPSHOT_GLOB = "components/*/work/jira-snapshot.yaml"
...
def test_snapshot_lint_wired_into_lint_repo(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/work/jira-snapshot.yaml", "feature: x\nissues: []\n")
    errors, _ = lint_repo(root)
    assert any("missing generated-file marker" in e for e in errors)
```
...
**Interfaces:**
- Consumes: `hublib.jira.client_from_env/probe_public/adf_to_text` (Task 1), `hublib.jiramap` (Task 2), the optional `jira:` block on `components/components.yaml` entries (`{jql: str, ref_types: [str]}`).
...
def _jira_cfg(root, feature):
    """The feature's jira: block from components.yaml; None = unknown feature,
...
async def _sync(root, feature, out, today):
    ...
    if feature:
        ...
        if not feats:
            print(f"ERROR '{feature}' has no jira: block in components/components.yaml "
...
---
name: hub.jira-sweep
description: Sweep Jira into the hub for one feature — conversational scope discovery (JQL stored in components.yaml), a curated public snapshot under <feature>/work/, and gated ref- entries for strategic issues (default Outcome/Feature types). Use when the user says "sweep jira for <feature>", "pull the jiras for <feature>", "set up jira tracking for <feature>", or when filed Jira links deserve field ingestion. Read-only against Jira; every repo write is gated.
---

# hub.jira-sweep

Input: a feature id (components/components.yaml) + optional JQL/scope hints.
...
2. SCOPE: a `jira:` block on the feature in components/components.yaml → use
   ...
   components.yaml edit proposed AT THE GATE (step 5) — nothing is written
...
5. GATE: one batch table — every proposed write, one line:
   ...
   (components/<id>/work/jira-snapshot.yaml, redacted count called out),
   each ref-, and the components.yaml scope edit when new/changed. Full
