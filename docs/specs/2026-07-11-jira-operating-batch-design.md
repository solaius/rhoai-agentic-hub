...
1. **One spec, three parts, sequenced by ascending risk.** #30 (read-only
   ...
   same client, the same `components.yaml` JQL scope, the same inline gate), so
...
4. **The full-fidelity triage report is restricted; the tracked record is
   ...
   report is written to `restricted/components/<f>/work/` (gitignored, full
   fidelity, no redaction needed because it never enters the public repo), and
   a tracked `components/<f>/work/triage-log.yaml` records the decisions with no
   Jira prose in it at all. The log satisfies the disclosure net by
   construction rather than by redaction.

5. **Triage scope reuses the `components.yaml` `jira:` scopes.** pm-toolkit
...
| `components/<f>/work/triage-log.yaml` | tracked, machine-written, prose free: the durable record of each triage pass |
| `restricted/components/<f>/work/triage-<date>.html` | gitignored, full fidelity: the working report |
...
`components/*/work/triage-log.yaml` to its scan tuple.

### `components/<f>/work/triage-log.yaml` (tracked, public, prose free)
...
`components.yaml jira: scope` + RFE filter → `hub_triage.py --scan` fetches (+
...
1. **PRE-FLIGHT.** `hub_jira.py --check`. On failure, stop and point at
   ...
   tracked path.
2. **SCOPE.** Read the feature's `jira: jql` from `components.yaml`; layer on
...
4. **REVIEW.** Report written to
   `restricted/components/<f>/work/triage-<date>.html`. Tell the human to open
...
- `hublib/disclosure.py`: `components/*/work/triage-log.yaml` added to
