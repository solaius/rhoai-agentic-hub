...
- **Closes:** enhancements backlog #30 (fully), #27(b) (fully), #29 (partially:
  the write surface is deliberately narrowed, see decision 2). Builds entirely
  on `scripts/hublib/jira.py`, which shipped 2026-07-10 with #2.
...
2. **The write door opens narrowly: labels and comments only, always gated.**
   Every Jira skill in the hub today advertises "Read-only against Jira" and
   nothing calls the client's write methods. This batch changes that, and the
   change is bounded by owner ruling: `hub.jira-triage` may **add labels** and
   **post comments**. It may not transition issues, change assignees, edit
   fields, or create issues. Consequences: pm-toolkit's `close`, `approve`,
   `assign` and `claim_component` actions are not ported; #30 loses its Create
   mode entirely and becomes audit-only; the `create_link` method that
   pm-toolkit references but never implements does not need to be written.
   The write surface stays in exactly one skill, so the posture is legible at
   the point of use.

3. **The full-fidelity triage report is restricted; the tracked record is
...
4. **Triage scope reuses the `features.yaml` `jira:` scopes.** pm-toolkit
...
5. **No LLM-provider credentials, so comment digests are dropped.**
...
6. **The browser is the input device; the gate is the authority.** Clicking
...
7. **The suggestion engine is ported; the markdown report is not.**
...
8. **Two upstream bugs are fixed rather than ported.** `assign_issue` sends
...
| path | holds |
|---|---|
| `scripts/hublib/triage.py` | pure functions (`flag_staleness`, `classify_rfe`, `suggest_action`), the RFE fetch, and `apply_decisions` (labels and comments only) |
...
- key: RHAIRFE-2630
  ...
  suggestion: backlog
  decision: backlog
  outcome: applied          # applied | skipped | error
...
No summaries, no comment text, no assignee names: nothing here is Jira prose,
so there is nothing for the probe rule to redact. Field order is fixed and the
file is byte-stable (`yaml.safe_dump(sort_keys=False)`), matching
`jira-snapshot.yaml`.
...
### Label vocabulary (the entire write surface)

| action | Jira effect |
|---|---|
| `roadmap` | adds `<release>-candidate` |
| `backlog` | adds `pm-backlogged` |
| `needs-uxd` | adds `needs-uxd` |
| `clarify` | adds `pm-needs-clarification` |
| `comment` | posts a comment (free text, shown in full at the gate) |
| `skip` | nothing |

Any other action value in a decisions JSON (`close`, `approve`, `assign`,
`claim_component`) is **rejected at the gate with a reason**, never silently
dropped. `<release>-committed` is read-only by policy and is never written.
...
5. **GATE.** Read the exported decisions JSON. Present one batch table, one
   line per Jira mutation: `RHAIRFE-2630: +label pm-backlogged`,
   `RHAIRFE-2644: comment "<full text>"`. Approve/edit/reject per line.
   Nothing touches Jira before OK. Unsupported actions are shown as rejected,
   with the reason.
6. **APPLY.** `apply_decisions`, per-item try/except, returns
   `{applied, skipped, errors}`. A label already present is an idempotent
...
7. **RECORD.** Write `work/triage-log.yaml`. Run `hub_index.py` then
   ...
   Report applied/skipped/errors counts.
...
- `docs/enhancements.md`: #30 and #27 move to Done; #29 moves to Done with an
  explicit note that transitions, assignment and issue creation were ruled out
  of scope (decision 2) and that cross-component discovery and comment digests
  were deferred (decisions 4 and 5), so a future session does not read "Done"
  as "fully ported".
- `memory/log.md`: a ship line. The write-posture change (decision 2) also
  warrants a durable fact, since it reverses a documented invariant.
...
- **Decisions JSON carries an unsupported action:** rejected at the gate, with
...
- **Partial apply failure:** the remaining items still apply; the run returns a
  coherent applied/skipped/errors split and the log records per-issue outcome.
...
- `test_triage.py`: the pure functions first (`flag_staleness` thresholds,
  ...
  most likely to be wrong. Then `apply_decisions`: a rejected action issues no
  request at all; a label write carries the full existing array; a partial
  failure returns a coherent split.
...
a known-messy issue; a `jira-gap` run on a feature with a live scope; and a
real triage pass ending in an actual gated write to Jira. The third is the one
that matters most: it is the first mutation this hub has ever made, and the
gate is the only thing standing between a misread JSON and production Jira.
