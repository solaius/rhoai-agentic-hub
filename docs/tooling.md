...
| `schema.py` | `lint_repo()` and `validate_manifest()`; owns the type vocabularies, filename-prefix map, skeleton contract, canonical-URI patterns, and the line budgets; validates `prototype/` directories: `prototype.yaml` required fields (title, description, status, current, versions, components), status enum, `current` points to existing directory, version dirs contain `index.html`, component ids are valid |
| `indexer.py` | deterministic generation of every `index.md` and `views/` file from frontmatter; the `<!-- generated … -->` marker; `write_all` / `check`; scans `components/*/prototype/*/prototype.yaml` and `narrative/prototype/*/prototype.yaml`, generates `views/prototypes.md` (cross-cutting prototype index), adds a Prototypes section to per-component `index.md` files |
