...
| `schema.py` | `lint_repo()` and `validate_manifest()`; owns the type vocabularies, filename-prefix map, skeleton contract, canonical-URI patterns, and the line budgets; validates `prototype/` dirs -- v2 files (with `branch:`) need source_repo, base, preview_url, current in versions, and a `commit` per version |
