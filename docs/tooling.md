...
| `python scripts/hub_triage.py --apply <decisions.json> --rows <rows.json> --component <c> --out <dir>` | the only mode that writes: applies labels/comments/transitions to Jira, writes the proposed `triage-log-<component>.yaml` into `<dir>` | driven by hub.jira-triage step 5; **writes to Jira** |
...
| `schema.py` | `lint_repo()` and `validate_manifest()`; owns the type vocabularies, filename-prefix map, skeleton contract, canonical-URI patterns, and the line budgets; validates `prototype/` dirs -- prototype.yaml files need source_repo, base, preview_url, current in versions, and a `commit` per version; optional `target` must be a key of `conventions/prototype-targets.yaml` and `composes` a list of branch names |
| `indexer.py` | deterministic generation of every `index.md` and `views/` file from frontmatter; the `<!-- generated … -->` marker; `write_all` / `check`; scans `components/*/prototype/*/prototype.yaml` and `narrative/prototype/*/prototype.yaml`, generates `views/prototypes.md` (cross-cutting prototype index), adds a Prototypes section to per-component `index.md` files; v2 prototypes link their live `preview_url` in `views/prototypes.md` and component indexes |
