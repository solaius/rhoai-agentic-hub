...
([fact-openshell-vs-agent-substrate](/features/agent-interop/knowledge/fact-openshell-vs-agent-substrate.md)).
Security + compliance differentiation compensates for RHOAI's 12-18 month
time-to-market gap versus Azure/AWS/Google
([00-executive-summary](/features/agent-interop/research/00-executive-summary.md)).
...
- Pre-deployment agent registry/catalog/discovery → [agent-registry](/features/agent-registry/)
- Starter-kit templates and catalog UI → [agent-catalog](/features/agent-catalog/)
- Agent observability/SDLC oversight → [agent-ops](/features/agent-ops/)
- MCP-level tool traffic governance → [mcp-gateway](/features/mcp-gateway/)
...
([05-landscape](/features/agent-interop/research/05-landscape.md)). No
...
([fact-openshell-architecture](/features/agent-interop/knowledge/fact-openshell-architecture.md)).
NVIDIA co-engineering and the kubernetes-sigs/agent-sandbox SIG provide the
upstream foundation
([fact-nvidia-co-engineering-model](/features/agent-interop/knowledge/fact-nvidia-co-engineering-model.md)).
...
3. **Customer**: 88% of agent pilots fail at security; enterprise sandbox
   ...
   ([ref-email-openshell-weekly-jun15](/features/agent-interop/knowledge/ref-email-openshell-weekly-jun15.md))

## Where we stand

**Decisions to date:**
- 2026-07-17: [Postgres as DP backend DB](/features/agent-interop/knowledge/decision-openshell-dp-postgres.md)
- 2026-07-17: [Base image dual path (Hummingbird + UBI/RHEL)](/features/agent-interop/knowledge/decision-openshell-base-image-dual-path.md)
- 2026-07-17: [GPU and confidential computing deferred past Oct/Nov](/features/agent-interop/knowledge/decision-openshell-defer-gpu-cc.md)
- 2026-07-11: Convergence onto OpenShell ([ref-email-openshell-convergence](/features/agent-interop/knowledge/ref-email-openshell-convergence.md))
...
- Layered sandboxing (OpenShell + Kata) validated on OpenShift 4.21
  ([ref-rh-developer-layered-sandboxing](/features/agent-interop/knowledge/ref-rh-developer-layered-sandboxing.md))
- Agent Sandbox SIG CRDs at v1beta1; API still changing fast
  ([ref-agent-sandbox-sig](/features/agent-interop/knowledge/ref-agent-sandbox-sig.md))
...
|-----|---------------|----------|
| [Privileged SCC on OpenShift](/features/agent-interop/knowledge/question-openshell-privileged-scc.md) | Enterprise customers flag as red flag; 3 mitigation paths identified (sidecar, user namespaces K8s 1.33+, layered Kata) but none production-validated on OCP yet | OpenShell #899, #1959; no RHAISTRAT |
| [No FIPS path](/features/agent-interop/knowledge/question-openshell-rust-fips.md) | Rustls + ring not FIPS-validated; FIPS 140-2 sunsets Sep 2026; blocks regulated customers. Phase 1 design specified (aws-lc-rs), SSH gap remains | OpenShell #900; no RHAISTRAT |
| [Multi-tenancy not implemented](/features/agent-interop/knowledge/question-openshell-multi-tenancy.md) | No per-tenant namespace scoping, quotas, or isolation. Recommended model exists but no implementation. Helm namespace split (#2485) is prerequisite | OpenShell #1722; no RHAISTRAT |
| [Gateway HA / leader election](/features/agent-interop/knowledge/question-openshell-gateway-ha.md) | Gateway is single point of failure; no documented HA, leader election, or HPA guidance. Multi-replica works but without guarantees | OpenShell #1012; no RHAISTRAT |

**High (enterprise readiness):**

| Gap | Why it matters |
|-----|---------------|
| [SKU / product home decision](/features/agent-interop/knowledge/question-openshell-sku-product-home.md) | RHOAI component vs separate operator vs OCP component — affects release cadence, engineering ownership, and entitlement model |
| [SPIFFE identity provisioning](/features/agent-interop/research/06-jira-gap.md) (G4) | RHAISTRAT-1730 covers authorization but NOT identity provisioning (SVID issuance, dynamic registration); the zero-trust foundation beneath authorization is untracked |
| [No Vault/KMS integration](/features/agent-interop/research/08-operations.md) | Provider credentials rotation is manual or OAuth2-only; no external secret store integration |
| [No official air-gap path](/features/agent-interop/research/07-deployment.md) | Practical approach works but is undocumented (NemoClaw #2218) |
| [Declarative config path](/features/agent-interop/knowledge/question-openshell-declarative-agent-config.md) | No CRD or K8s-native declarative path today; CRD consensus exists but design untracked |
...
- **Support boundary**: harness binaries are "validated, not supported" ([fact-harness-support-boundary](/features/agent-interop/knowledge/fact-harness-support-boundary.md)) — customers may not understand the distinction

## Jira map

### Coverage (from ref- entries and [06-jira-gap](/features/agent-interop/research/06-jira-gap.md))
...
| Aug 2 2026 | EU AI Act high-risk obligations start | Agent transparency requirements become enforceable; eight-field agent record becomes table stakes ([fact-agent-registry-regulatory-record-fields](/features/agent-registry/knowledge/fact-agent-registry-regulatory-record-fields.md)) |
