...
|---|---|
| The AI data plane (inference routing, agentic loop, API translation, guardrails, credentials, metering) | MCP protocol traffic governance (→ [mcp-gateway](/features/mcp-gateway/knowledge/fact-mcp-gateway.md), converging via same migration track post-3.6) |
...
| Agent governance at the network plane (identity, auth, audit in transit) | Agent sandboxing and execution isolation (→ [agent-interop](/features/agent-interop/knowledge/fact-agent-interop-overview.md) / OpenShell) |
...
([decision-praxis-convergence](/features/ai-gateway/knowledge/decision-praxis-convergence.md)).
...
([02-competitive](/features/ai-gateway/research/02-competitive.md),
Section 5).
...
the gap ([02-competitive](/features/ai-gateway/research/02-competitive.md),
Section 9).
...
| 2026-07-06 | AI Gateway team formed — first networking→AI BU team | [fact-ai-gateway-team](/features/ai-gateway/knowledge/fact-ai-gateway-team.md) |
| 2026-07-30 | Praxis convergence: one router, one policy, one config | [decision-praxis-convergence](/features/ai-gateway/knowledge/decision-praxis-convergence.md) |
...
|---|---|---|
| [Agentic loop x filter pipeline ordering](/features/ai-gateway/knowledge/question-ai-gateway-agentic-loop-ipp.md) | Different orderings needed for outer gateway vs inner loop | Research says Praxis makes this structural; needs validation |
| [Conversation state management](/features/ai-gateway/knowledge/question-ai-gateway-conversation-state.md) | Auto-compaction for server-side Responses | No model cooperation for self-hosted; Praxis must implement |
| [Cross-DC rate limiting](/features/ai-gateway/knowledge/question-ai-gateway-cross-dc-rate-limiting.md) | Single user's budget across multiple DCs | Split-budget with reconciliation is pragmatic approach |
| [Cost multiplication (selection x fan-out)](/features/ai-gateway/knowledge/question-ai-gateway-model-selection-cost.md) | Failover + ITS fan-out compounds costs silently | Pre-flight budget checks needed; no Jira tracks this |
| [Tenancy x MCP Registry governance](/features/ai-gateway/knowledge/question-ai-gateway-tenancy-mcp-registry.md) | Group-based tenancy must compose with Registry governance | Needs ABAC spike (RHAISTRAT-2409) to land first |

### Research-identified risks

| Risk | Severity | Detail |
|---|---|---|
| CNCF sandbox GPLv3 friction | High | Most CNCF projects use Apache-2.0; license may block acceptance ([04-upstream](/features/ai-gateway/research/04-upstream.md)) |
| Istio integration race | High | agentgateway has experimental Istio 1.30 integration; Microsoft paving the path. Praxis has no upstream Istio contribution tracked ([05-jira-gap](/features/ai-gateway/research/05-jira-gap.md)) |
| FIPS 140-3 for Rust | High | No pure-Rust crypto has FIPS validation. FIPS 140-2 sunsets Sep 2026. Delivery risk for 3.6 ([03-requirements](/features/ai-gateway/research/03-requirements.md)) |
...
| Provider coverage gap | Medium | 4 providers vs LiteLLM's 140+. Community contribution model needed post-3.6 ([02-competitive](/features/ai-gateway/research/02-competitive.md)) |

### Execution risks (from prior F2F)

- Conversation state continuity across 3.5→3.6 engine swap — explicit open item, no owner ([fact-ai-gateway-execution-risks](/features/ai-gateway/knowledge/fact-ai-gateway-execution-risks.md))
...
Gaps identified by the [jira-gap analysis](/features/ai-gateway/research/05-jira-gap.md) with no current Jira work:
