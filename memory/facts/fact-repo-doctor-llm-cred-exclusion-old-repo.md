...
---
Describes the OLD `ai-asset-registry` repo's tooling, not this hub. Resolved 2026-07-08 when the R4 wave-4 MCP-server sections landed: the hub does **not** need (or carry) the same guard — see the closing ruling below.
...
Don't re-add LLM-provider vars to anything persisted to shell/Claude config, in that repo or this hub. Hub-side ruling (owner, 2026-07-08): the hub carries **none** of this machinery in any form — not even the old doctor's defensive `unset` — because hub users arrive with Claude Code/Cursor already configured and the hub's `restricted/.env` never carries LLM-provider keys; keep it that way. See [fact-repo-doctor-tracker-gap-old-repo.md](/memory/facts/fact-repo-doctor-tracker-gap-old-repo.md).
