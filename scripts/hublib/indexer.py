...
def _load_entries(root, subdir_glob, base="components"):
...
def _home(rootpath):
    """Group label for an entry: its component id, or 'narrative'."""
    parts = rootpath.split("/")
    return parts[2] if parts[1] == "components" else parts[1]
...
def build_all(root, today=None):
    ...
    # components/index.md
    lines = [MARKER + "# Components", ""]
    for f in comps:
        lines.append(f"- [{f.get('title', f['id'])}](/components/{f['id']}/index.md)"
                     f" — {f.get('description', '')}")
    if not comps:
    ...
    for f in comps:
        ...
        if (fdir / "knowledge").is_dir():
            ...
            for etype in sorted(by_type):
                ...
                lines.append("")
            built[f"components/{f['id']}/knowledge/index.md"] = \
                "\n".join(lines).rstrip("\n") + "\n"
    ...
    for prp, pm in pillars:
        ...
        for rp, m in owned:
            ...
            comps_str = ", ".join(f"[{fid}](/components/{fid}/index.md)"
                                  for fid in (m.get("components") or []))
            if comps_str:
                lines.append(f"  - connects: {comps_str}")
        if not owned:
