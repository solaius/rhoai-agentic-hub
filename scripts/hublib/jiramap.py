...
def build_snapshot(component_id, jql, rows, swept):
...
def validate(root):
    ...
    for snap in sorted(root.glob(SNAPSHOT_GLOB)):
        ...
        if data.get("component") != fid:
            errors.append(f"{rel}: component '{data.get('component')}' does not match "
                          f"its directory '{fid}'")
