...
def test_valid_entry_passes(tmp_path):
    ...
    write(root, "components/mcp-registry/knowledge/fact-scope.md", ENTRY.format(t="fact", extra=""))
...
def test_prefix_type_mismatch_is_error(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/knowledge/ref-a.md", ENTRY.format(t="fact", extra=""))
    errors, _ = lint_repo(root)
...
def test_reference_requires_canonical_resource(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/knowledge/ref-a.md",
...
def test_stray_dir_in_component_is_error(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/notes/a.md", ENTRY.format(t="fact", extra=""))
    errors, _ = lint_repo(root)
    assert any("not part of the component skeleton" in e for e in errors)
...
def test_broken_root_link_is_warning(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/knowledge/fact-a.md",
          ENTRY.format(t="fact", extra="") + "see [gone](/components/x/knowledge/missing.md)\n")
    errors, warnings = lint_repo(root)
    assert errors == []
...
def test_manifest_duplicate_dest_is_error(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/enablement/deck/index.html", "<html></html>")
    write(root, "publish/manifest.yaml",
          "- source: components/x/enablement/deck/\n  dest: a/\n  audience: public\n"
          "  title: T\n  description: D\n"
          "- source: components/x/enablement/deck/\n  dest: a/\n  audience: public\n"
          "  title: T2\n  description: D2\n")
...
def test_manifest_missing_source_is_error(tmp_path):
    root = make_repo(tmp_path)
    write(root, "publish/manifest.yaml",
          "- source: components/x/enablement/nope/\n  dest: a/\n  audience: public\n"
          "  title: T\n  description: D\n")
    errors = validate_manifest(root)
...
def test_pillar_and_story_invalid_under_components(tmp_path):
    root = make_repo(tmp_path)
...
def test_known_component_ids_pass(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/components.yaml", COMPONENTS_YAML)
    write(root, "components/mcp-registry/knowledge/fact-a.md",
          ENTRY.format(t="fact", extra="components: [mcp-registry, mcp-gateway]\n"))
    errors, _ = lint_repo(root)
    assert errors == []


def test_unknown_component_id_is_error(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/components.yaml", COMPONENTS_YAML)
    write(root, "components/mcp-registry/knowledge/fact-a.md",
          ENTRY.format(t="fact", extra="components: [mcp-registry, made-up]\n"))
    errors, _ = lint_repo(root)
...
def test_related_unknown_id_is_error(tmp_path):
    ...
    write(root, "components/components.yaml",
          ...
          "  related: [made-up]\n")
    errors, _ = lint_repo(root)
    assert any("unknown related component id 'made-up'" in e for e in errors)
...
def test_related_self_reference_is_error(tmp_path):
    ...
    write(root, "components/components.yaml",
          ...
          "  related: [mcp-registry]\n")
    errors, _ = lint_repo(root)
    assert any("must not include the component itself" in e for e in errors)
...
def test_components_must_be_a_list(tmp_path):
    ...
    write(root, "components/components.yaml", COMPONENTS_YAML)
    write(root, "components/mcp-registry/knowledge/fact-a.md",
          ENTRY.format(t="fact", extra="components: mcp-registry\n"))
    errors, _ = lint_repo(root)
    assert any("components must be a list" in e for e in errors)


def test_components_on_memory_file_is_error(tmp_path):
    root = make_repo(tmp_path)
...
def test_narrative_story_and_pillar_valid(tmp_path):
    ...
    write(root, "narrative/knowledge/story-governed-mcp.md",
          ENTRY.format(t="story",
                       extra="components: [mcp-registry, mcp-gateway]\n"
...
def test_story_requires_components(tmp_path):
    root = make_repo(tmp_path)
    write(root, "narrative/knowledge/story-a.md", ENTRY.format(t="story", extra=""))
    errors, _ = lint_repo(root)
    assert any("type 'story' requires field 'components'" in e for e in errors)
...
def test_story_dangling_pillar_is_warning(tmp_path):
    ...
    write(root, "narrative/knowledge/story-a.md",
          ENTRY.format(t="story", extra="components: [mcp-registry]\n"
...
def test_artifact_descriptor_valid(tmp_path):
    ...
    write(root, "components/mcp-registry/enablement/deck/artifact.md",
          ENTRY.format(t="artifact", extra="components: [mcp-gateway]\n"))
    errors, _ = lint_repo(root)
    assert errors == []
...
def test_story_invalid_under_components(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/knowledge/story-a.md", ENTRY.format(t="story", extra=""))
    errors, _ = lint_repo(root)
...
def test_narrative_artifact_descriptor_valid(tmp_path):
    ...
    write(root, "narrative/enablement/deck/artifact.md",
          ENTRY.format(t="artifact", extra="components: [mcp-registry]\n"))
    errors, _ = lint_repo(root)
    assert errors == []
...
def test_snapshot_lint_wired_into_lint_repo(tmp_path):
    root = make_repo(tmp_path)
    write(root, "components/x/work/jira-snapshot.yaml", "component: x\nissues: []\n")
    errors, _ = lint_repo(root)
...
def test_restricted_tree_skipped_when_encrypted(tmp_path):
    ...
    root = make_repo(tmp_path)
    know = tmp_path / "restricted" / "components" / "x" / "knowledge"
...
def test_restricted_tree_handles_mixed_plaintext_leftover_and_encrypted_blob(tmp_path):
    ...
    (know / "fact-a.md").write_bytes(b"\x00GITCRYPT\x00\x00\x02\x00" + b"\xff" * 50)
    write(root, "restricted/components/x/knowledge/fact-b.md",
          "---\ndescription: d\ntimestamp: 2026-07-05\n---\nbody\n")
    errors, warnings = lint_repo(root)
